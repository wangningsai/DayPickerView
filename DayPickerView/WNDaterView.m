...
@interface WNDaterView ()<UIPickerViewDelegate,UIPageViewControllerDataSource>
{
    ...
    NSMutableArray *miniuterArr;
}
@end
@implementation WNDaterView
-(void)initData{
    ...
    miniuterArr = [NSMutableArray array];
    for (int i = 0; i<24; i++) {
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component __TVOS_PROHIBITED{
    if (_dateViewType == WNDaterViewTypeDate) {
        NSUInteger mid = 0;
        ...
        self.dateString = [NSString stringWithFormat:@"%d-%02d-%02d",_year,_month,_day];
    }else if (_dateViewType == WNDaterViewTypeTime){
        NSUInteger mid = 0;
        NSUInteger rowIndex = 0;
        if (component==0) {
            mid = [self midNum:24];
            ...
            rowIndex = row%60;
            [picker selectRow:rowIndex+mid inComponent:1 animated:NO];
            _miniute = [miniuterArr[rowIndex] intValue];
        }else if (component==2){
            mid = [self midNum:60];
            rowIndex = row%60;
            [picker selectRow:rowIndex+mid inComponent:2 animated:NO];
            _second = [miniuterArr[rowIndex] intValue];
        }
        self.timeString = [NSString stringWithFormat:@"%d:%02d:%02d",_hour,_miniute,_second];
    }
...
-(UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(UIView *)view{
    ...
    if (_dateViewType == WNDaterViewTypeDate) {
        ...
        if(component == 0){
            ...
            rowLab.text = [NSString stringWithFormat:@"%@分",miniuterArr[row%60]];
        }else{
            rowLab.text = [NSString stringWithFormat:@"%@秒",miniuterArr[row%60]];
        }
    }
...
- (void)setDateViewType:(WNDateViewType)dateViewType{
